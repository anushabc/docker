...
1. Disconnect your ubuntu container froom bridge network by running the below command

   ```
   docker network disconnect bridge networktest
   ```

2. Create your own bridge network by running the below command:

   ```
   docker network create -d bridge my_bridge
   ```
   
 3. Run ```docker network ls``` and verify your network is listed.

    ```
    docker network ls
    ```
    
 4. Now inspect your network and note there no containers associated to it.
